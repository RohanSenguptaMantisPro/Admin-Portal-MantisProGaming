import 'package:admin_portal_mantis_pro_gaming/src/authentication/domain/repo/auth_repo.dart';
import 'package:admin_portal_mantis_pro_gaming/src/authentication/domain/usecases/cache_user_token.dart';
import 'package:dartz/dartz.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mocktail/mocktail.dart';

import 'auth_repo.mock.dart';

void main() {
  late AuthRepo authRepo;
  late CacheUserToken cacheUserToken;

  setUp(() {
    authRepo = MockAuthRepo();
    cacheUserToken = CacheUserToken(authRepo);
  });

  const tUserToken = 'test user token';

  test(
    'should call the [AuthRepo.cacheUserToken]',
    () async {
      when(() => authRepo.cacheUserToken(any())).thenAnswer(
        (_) async => const Right(null),
      );

      final result = await cacheUserToken(tUserToken);

      expect(result, equals(const Right<dynamic, void>(null)));

      verify(() => authRepo.cacheUserToken(tUserToken)).called(1);

      verifyNoMoreInteractions(authRepo);
    },
  );
}
